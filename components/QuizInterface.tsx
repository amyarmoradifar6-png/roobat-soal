...
export const QuizInterface: React.FC<QuizInterfaceProps> = ({ initialChapter, onBack }) => {
    ...
    return (
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 md:p-8 transition-colors">
        <div className="text-center mb-8">
          <div className="inline-block p-4 rounded-full bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 mb-4">
        ...
        <div className="space-y-6">
          {state.questions.map((q, idx) => {
            ...
            return (
              <div key={idx} className={`p-4 rounded-xl border ${isCorrect ? 'border-green-200 bg-green-50 dark:bg-green-900/20 dark:border-green-800' : 'border-red-200 bg-red-50 dark:bg-red-900/20 dark:border-red-800'}`}>
  ...
  return (
    <div className="max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {/* Header Info */}
      <div className="mb-4 flex justify-between items-center text-slate-500 dark:text-slate-400 font-bold text-sm md:text-base">
      ...
      {/* Question Card */}
      <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-sm border border-slate-200 dark:border-slate-800 p-6 md:p-10 min-h-[400px] flex flex-col transition-colors">
        ...
        <div className="space-y-4 flex-1">
          {currentQuestion.options.map((option, idx) => (
            <button
              ...
              className="w-full p-4 rounded-xl text-right border transition-all duration-200 flex items-center justify-between group bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 border-slate-200 dark:border-slate-700"
            >
        ...
        <div className="flex justify-between mt-10 pt-6">
          ...
          <button
            ...
            className="px-8 py-3 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-indigo-200 dark:shadow-none font-bold text-sm"
          >
